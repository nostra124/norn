...
size_t dhtstore_init(int budget_mb, int client_only) {
    ...
    if (budget_mb > 0) {
        ...
        size_t b = ram ? ram / 512 : 2 * 1024 * 1024;  /* ~0.2% of RAM, 2MB fallback */
        if (b < 2 * 1024 * 1024)  b = 2 * 1024 * 1024;
        if (b > 64 * 1024 * 1024) b = 64 * 1024 * 1024;
        g_budget = b;
    }
    return g_budget;
}
...
static item_t *find(const unsigned char target[20]) {
    for (int i = 0; i < g_count; i++)
        if (g_items[i].used && memcmp(g_items[i].target, target, 20) == 0) return &g_items[i];
    return NULL;
}
...
static void expire(long now) {
    for (int i = 0; i < g_count; ) {
        if (now - g_items[i].stored > DHTSTORE_TTL) remove_at(i);
        else i++;
    }
}
...
static int per_ip_count(uint32_t ip) {
    int n = 0;
    for (int i = 0; i < g_count; i++) if (g_items[i].used && g_items[i].src_ip == ip) n++;
    return n;
}
...
static int store_item(const unsigned char target[20], const unsigned char k[32],
    ...
    if (e) {
        ...
        while ((g_bytes + vlen > g_budget || g_count >= DHTSTORE_MAX) && g_count > 0)
            evict_lru();
        if (g_bytes + vlen > g_budget) return 0;         /* item alone exceeds budget */
        e = &g_items[g_count++];
...
int dhtstore_put_immutable(const unsigned char *v, size_t vlen, uint32_t src_ip,
    ...
    if (bep44_immutable_target(v, vlen, tgt) != 0) return 0;   /* >1000B: reject (BUG-059) */
...
int dhtstore_del(const unsigned char target[20]) {   /* BUG-122 */
    if (!target) return 0;
    for (int i = 0; i < g_count; i++)
        if (g_items[i].used && memcmp(g_items[i].target, target, 20) == 0) {
            remove_at(i);
            return 1;     /* removed */
        }
    return 0;             /* not held */
